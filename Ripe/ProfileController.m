...
@interface ProfileController ()
{
    ...
    NSArray * ratingList;
    ProfilePhotosController * photoController;
    NSArray * reviewList;
}
@end
...
- (void)viewDidLoad
{
  ...
  self.usernameField.text=  [DataManager shared].firstName;
    ...
    photoController=[[ProfilePhotosController alloc]initWithStyle:UITableViewStyleGrouped];
...
- (void)DKScrollingTabController:(DKScrollingTabController*)controller selection:(NSUInteger)selection
{
    if(selection==0)
    {
       ...
       [self checkAndRemove:self.selectionView andChildView:photoController.view];
    }
    else if(selection==1)
    {
